...
> **作用范围**: 本项目专属文档  
> **最后更新**: 2026-01-07 (定时提醒功能修复 - reminderAgentManager)  
...
### 2025-12-12 凌晨 (Map Kit 正确初始化 - MapsInitializer) 🔑
...
**待验证**:
1. 清理并重新构建（Build → Clean Project）
2. 卸载旧版本应用（避免缓存）
3. 运行到设备并观察日志：
   - ✅ `Map Kit 初始化成功`
   - ✅ `定位权限请求结果: ...`
   - ✅ `✅ 定位权限已授予`
   - ❌ 不应再有 `403 queryTile error`

**后续清理步骤**（如仍有问题）:
```powershell
# 卸载旧应用
hdc uninstall com.eric.PollenForecast

# 清除 AGC 缓存
hdc shell rm -rf /data/accounts/account_0/applications/com.eric.PollenForecast
...
### 2025-12-12 凌晨 (Map Kit API Key 配置) 🔑 [已废弃]

**问题背景**:
- AGC 配置文件已更新，动态权限请求已添加
- 地图仍然 403 错误：`queryTile error: no map permission!`（重复 70+ 次）
- 日志中未看到权限请求相关输出

**新发现**:
- 用户提供了独立的 Map Kit API Key: `17EA94E3728228DDAB62B445ECD97129A90D3C0EB5F86F66D961D3EC03A531B2`
- 此密钥可能是从 AGC 控制台单独获取的明文密钥

**解决方案**:
- ✅ **在代码中显式设置 API Key**:
  - 在 `MapView.aboutToAppear()` 中调用 `map.setApiKey()`
  - 在地图初始化前设置密钥
  - 添加日志输出以验证设置成功

**修改文件**:
- `entry/src/main/ets/views/MapView.ets` (添加 API Key 设置)

**代码变更**:
```typescript
aboutToAppear(): void {
  // 设置 Map Kit API Key（必须在地图初始化前设置）
  try {
    map.setApiKey('17EA94E3728228DDAB62B445ECD97129A90D3C0EB5F86F66D961D3EC03A531B2');
    console.info('[MapView] Map Kit API Key 已设置');
  } catch (err) {
    console.error('[MapView] 设置 API Key 失败:', JSON.stringify(err));
  }
  // ... 其他初始化代码
}
```
...
- 重新构建并运行到设备
- 查看日志是否有 `Map Kit API Key 已设置`
- 确认地图是否正常加载（403 错误应消失）

**技术要点**:
- `map.setApiKey()` 必须在 MapComponent 初始化前调用
- API Key 来源可能是 AGC 控制台 → 项目设置 → API管理 → Map Kit
- 区别于 agconnect-services.json 中的加密 api_key 字段

---

### 2025-12-12 凌晨 (Map Kit 403 错误修复 - AGC 配置更新) 🗺️

**问题背景**:
- 地图显示空白，日志显示 `403 queryTile error: no map permission!`（重复 30+ 次）
- 错误 8521226: `com.huawei.hms.mapservice.kit` 无法找到（重复 5 次）
- SHA-256 证书指纹已添加到 AGC，但地图仍无法加载

**根本原因**:
- **AGC 配置文件未同步**: `figma/agconnect-services.json` 与 `entry/src/main/resources/rawfile/agconnect-services.json` 哈希值不同
- 实际使用的 rawfile 版本是旧版本，不包含新添加的证书指纹授权信息
- 缺少动态权限请求代码（声明权限 ≠ 运行时授权）

**解决方案**:
1. ✅ **替换 AGC 配置文件**:
   - 用 `figma/agconnect-services.json` 替换 `rawfile/agconnect-services.json`
   - 新版本包含更新后的证书指纹衍生值（code1-4）

2. ✅ **添加动态权限请求**:
   - 在 `EntryAbility.onCreate()` 中调用 `requestLocationPermissions()`
   - 使用 `abilityAccessCtrl.requestPermissionsFromUser()` 请求定位权限
   - 权限列表: `LOCATION` + `APPROXIMATELY_LOCATION`

**修改文件**:
- `entry/src/main/resources/rawfile/agconnect-services.json` (替换)
- `entry/src/main/ets/entryability/EntryAbility.ets` (添加权限请求)

**待验证**:
- 构建并运行到设备
- 检查是否弹出定位权限请求对话框
- 确认地图是否正常显示（不再有 403 错误）
- AGC 控制台确认 Map Kit 服务已开启

**技术要点**:
- AGC 配置文件必须在添加证书指纹后重新下载
- `abilityAccessCtrl.requestPermissionsFromUser()` 需在 Ability Context 下调用
- Map Kit 依赖项检查：项目暂未显式添加 `@hmscore/hms-map-kit` 依赖

---

### 2025-12-12 凌晨 (设置页弹窗修复 - CustomDialog 方案) ✨
...
**当前问题**:
- ⏳ Column布局下导航栏毛玻璃效果失效（已整理到当前问题.md等待CodeGenie回复）
...
### 2025-12-06 (API集成 - 续)
- **华为云部署**: 在亚太-新加坡区域创建 FunctionGraph 函数
- **问题发现**: 新加坡区域不支持 APIC 触发器
- **待解决**: 需要找到其他方式暴露函数的公网访问URL
- **备选方案**: 可考虑香港区域或使用函数自带的HTTP URL
...
### 2025-12-14 (地图定位按钮修复 - CodeGenie 审核优化版)
**问题描述**: 
...
### 2025-12-15 ⚠️ **ReminderAgentKit 紧急修复**
