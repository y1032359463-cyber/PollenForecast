...
**问题发现**：服务器端 Minshu API 返回 502/500 错误，且提示“API 配置缺失”。

**问题原因**：
1. **URL 格式错误**：敏舒接口实际路径为路径参数格式（`/day/pollen/{date}/{adcode}`），原代码错误使用了查询参数。
2. **环境加载缺失**：Node.js 原生不读取 `.env`，且项目未安装 `dotenv` 依赖，导致 API Key 读取失败。

**解决方案**：
1. ✅ **URL 路径重构**：修改 `qweather-proxy.js` 和 `minshu-batch.js`，适配敏舒官方 RESTful URL 格式。
2. ✅ **零依赖环境加载**：在脚本中实现 `loadEnv()` 函数，手动解析并加载 `.env` 文件到 `process.env`。
3. ✅ **SQLite 持久化增强**：启用本地数据库 `pollen_data.db` 存储 Minshu 数据，实现“查询优先本地，未命中回源”的缓存逻辑。

#### 第2轮：数据保留策略与自动化配置

**功能实现**：实现 7 天数据保留策略并配置自动化抓取任务。

**实施内容**：
1. ✅ **7 天清理逻辑**：在 `Database` 类中实现 `deleteDataBeforeDate` 方法，并在抓取任务结束时自动调用。
2. ✅ **定时任务配置**：使用 `crontab -e` 配置每天 23:55 执行 `minshu-batch.js`。
3. ✅ **环境兼容性处理**：
   - 确认 Node.js 实际路径：`/root/.nvm/versions/node/v20.19.6/bin/node`
   - 在 crontab 中使用绝对路径以确保执行成功。

**关键经验**：
- **路径敏感性**：定时任务执行环境与登录 shell 不同，必须使用 Node.js 的绝对路径。
- **日志隔离**：使用 `>> batch.log 2>&1` 导出日志，避免后台任务输出干扰主终端。
- **环境隔离**：在不增加 npm 依赖的情况下，手写 `.env` 加载器是轻量级代理服务的最佳实践。
...
**问题发现**：用户发现项目文件"变少"，实际是 `build-profile.json5` 被简化

**问题原因**：DevEco Studio 可能自动修改配置文件格式

**解决方案**：从备份 `PollenForecast_v1.0.2_20260110_144300` 恢复完整配置

**恢复内容**：
- 签名配置（signingConfigs）
- 产品配置（products: targetSdkVersion, compatibleSdkVersion）
- 构建模式（buildModeSet: debug/release）
- 模块配置（modules）

**验证成功**：配置文件已恢复 57 行完整格式

**关键教训**：
- 定期检查配置文件完整性
- 多版本备份的重要性（本次从第2个备份版本成功恢复）
- DevEco Studio 可能自动简化配置，需要人工检查

#### 第2轮：项目备份规则建立

**功能实现**：建立项目副本备份机制（既是备份也是demo）

**实施内容**：
1. ✅ **备份位置**：`.claude/backup/PollenForecast_v版本号_日期时间/`
2. ✅ **备份方式**：复制项目 → 删除不需要文件 → 完成
3. ✅ **创建文档**：`备份规则.md` 记录完整备份流程和常见问题
4. ✅ **区域Tab优化方案制定**：
...
**重要教训**：备份前必须确保原项目配置完整（`oh-package.json5` 需有 `modelVersion`，`hvigorfile.ts` 需使用 `appTasks`）
...
**功能实现**：客户端已支持敏舒花粉数据源，用户可选择使用国内权威数据

**实施内容**：
1. ✅ **创建工具类**
   - `AdcodeUtils.ets` - 城市编码工具类（城市名 → adcode 映射）
   - `MinshuDataSourceAdapter.ets` - 敏舒数据源适配器
2. ✅ **数据格式转换**：敏舒 API 格式 → 应用内部格式（category 中英文转换）
3. ✅ **更新服务层**
   - `PollenDataSource.ets` - 添加 MINSHU 数据源类型
   - `PollenService.ets` - 支持敏舒数据源调用
4. ✅ **AUTO 模式智能选择**：国内城市自动使用敏舒，其他使用 Google
...
**功能实现**：在广州和风天气服务器上新增 Minshu 花粉数据代理接口

**实施内容**：
1. ✅ **接口路径**: `/minshu/pollen/day?adcode={adcode}&recordDate={yyyy-MM-dd}`
2. ✅ **API Key**: `AK-lYpA8KnbT0lzPNvfjIqCSLDMLPz1n3htrENs`（截止 2026-07-01）
3. ✅ **授权范围**: 仅花粉数据（未授权城市过敏指数等其他接口）
4. ✅ **城市覆盖**: 全国省市区县（adcode 编码表见 `figma/日志.md`）
5. ✅ **首页数据来源标注**：
