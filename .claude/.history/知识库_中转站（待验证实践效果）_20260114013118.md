...
- ✅ **直辖市JSON数据结构特殊处理** → 已转移至 `知识库/ArkTS语法.md`
  - 北京/上海/天津/重庆特殊结构处理
- ✅ **三级菜单实现问题修复方案** → 已转移至 `知识库/UI组件.md`（2026-01-13完全修复）
  - 异步加载时机问题解决方案
  - 防御性编程最佳实践
  - 数据加载状态控制
- ✅ **华为Map Kit区县数据查询方案** → 已转移至 `知识库/第三方API集成.md`（已确认使用本地JSON）
  - Map Kit能力边界说明
  - 本地JSON方案实施建议
  - 性能优化策略
...
## 🔧 List组件点击事件绑定规范（三专家共识，2026-01-10）

> **来源**: 小艺 AI + CodeGenie + 小米MIMO
> **状态**: ✅ **问题根因已确认，待实施验证**
> **项目**: PollenForecast
> **问题**: 省份分组点击无法展开，onClick 事件不触发

### 一、核心问题：事件绑定位置错误（三专家100%共识）
...
#### 1. Set 类型问题
**两专家观点**：
- **小艺 AI**：`@State` 装饰的 `Set` 类型在 ArkUI 中无法正确触发 UI 更新
- **CodeGenie**：Set 类型的状态变更无法触发 ArkUI 响应式更新

**小米MIMO 观点**：
- Set 的 add/delete 方法**会触发** ArkUI 响应式更新
- 当前代码的 `@State expandedGroups: Set<string>` 方案正确，无需修改

#### 2. 改用数组方案（小艺 + CodeGenie 建议）
...
**改用数组**：
```typescript
@State expandedKeys: string[] = []  // 改用数组
...
### 四、事件绑定扩展配置（CodeGenie 建议）

```typescript
ListItem()
  .onClick(() => { ... })
  .hitTestBehavior(HitTestMode.Transparent)  // 允许穿透点击
  .clickEffect(ClickEffect.None)             // 禁用默认点击效果
```
...
3. **检查日志输出**：
   ```
   [点击生效] 北京, key: 北京
   [RegionView] toggleGroup 被调用，groupKey: 北京
   [RegionView] 展开后 expandedGroups: ["北京"]
   ```
4. **观导UI变化**：
   - 箭头从 `▶` 变为 `▼`
   - 城市列表展开显示
